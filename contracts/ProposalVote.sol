...
contract ProposalVote{
    ...
    enum PropsStatus {None, Created, pending, Accepted} // enum is to check the status action
    // we use struct to group related data
    struct Proposal {
        string name;
        string description;
        uint16 voteCount;
    ...
    // there are 3 locations where data can be fund in blockchain
    // stack, memorry and storage

    function createProposal(string memory _name, string memory _desc, uint16 _quorum) external  {
        require(msg.sender != address(0), "Zero address not allowed" );
        Proposal memory newProposal; // newinstance of the proposal struct
    ...
    function voteOnProposal(uint8 _index) external {
        ...
        if(currentProposal.voteCount >= currentProposal.quorum) {
            ...
            // emit its to log an event so that the frontend  can listening to events happening in the blockchain 
